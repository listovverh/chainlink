...
func (it *EvmChainReaderInterfaceTester[T]) GetChainReader(t T) clcommontypes.ChainReader {
	...
	lp := logpoller.NewLogPoller(logpoller.NewORM(it.Helper.ChainID(), db, lggr, pgtest.NewQConfig(true)), it.client, lggr, time.Millisecond, false, 0, 1, 1, 10000)
	require.NoError(t, lp.Start(ctx))

	cr, err := evm.NewChainReaderService(lggr, lp, it.client, it.chainConfig)
